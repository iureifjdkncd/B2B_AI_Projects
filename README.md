...
-	기여도
    ...
    → 실시간 사출 생산데이터 예측 과정에서 실시간 데이터의 품질저하/학습이력 유무에 강건한 지속가능 시스템 구축 (작업자 검사 기준 불량탐지 정확도 약 20% 향상)

    → 예측기반 과거 설비/품목당 최적생산 Setting 정보 구축으로 사용자 가이던스 제공 (생산 불량 발생률 약 10% 감소)
...
-	기여도
    ...
    → 데이터 품질문제(Ex.라벨링)을 고려한 AI & 통계기반 실시간 모니터링 시스템 구축  (작업자 검사 기준준 불량탐지 정확도 약 14% 향상)

    → 실시간 특성 반영한 군집기반 CoolingTime/InjectionTime_Mean Setting 참고용 모니터링 시스템 구축 (실제 공정조건 vs 추천 공정조건 정확도 약 2~3%가량 유지)
