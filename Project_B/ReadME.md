...
### 실시간 추론 프로세스 1 ( 최초 예측값 Ensemble )
  ...
  - 2.) 각 학습모델 기반 품질 예측값 KDE Ensemble & 품질 범위 정량화 (Data Drift Robustness 목표)
       ...
       → 점추정/확률 추정 결과들의 Min/Mean/Max 특징들로 KDE 모수 지정 & 분포 생성 및 Final Mean값 별도 추론
...
### 주요 성과 
   ...
   - 1.) AI시스템 완성 이후 점추정 Ensemble 예측 기준 제당공정 품질의 실시간 오차는 약 3 ~ 8% 유지

     (초기 품질 정확도는 약 90%이상 & 실제 적용 시 약 80%가량으로 수렴)

   - 2.) 예측 기반 목표값 대비 추천공정조건 및 작업자 기반 실제공정조건 간 오차는 약 5 ~ 10% 유지 
